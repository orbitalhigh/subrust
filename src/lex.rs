...
/// Scan a `"`-delimited body: `*i` points just past the opening quote on
/// entry, and just past the closing quote on return. Shared by string and
/// byte-string literals (same escape set). Emits diagnostics on errors.
...
/// Scan a byte literal body: `*i` points just past `b'`, and past the closing
/// `'` on return. One ASCII byte or an escape (same set as strings). Always
/// advances `*i` so the lexer makes progress even on a malformed literal.
...
/// Keyword lookup: subset keyword kind, T_KW_RESERVED for any other Rust
/// keyword (strict 2021 set + reserved-for-future), or T_IDENT.
...
/// Valid literal suffix check. Returns the token kind after applying the
/// suffix (`1f64` becomes a float), or T_EOF for an unsupported suffix.
...
/// Lex `src` into `mem.toks` (always terminated by a T_EOF token).
...
/// Human label for a token kind, used in composed diagnostics.
pub fn tok_label(k: u16) -> &'static str {
    match k {
        T_EOF => "end of file",
        T_IDENT => "an identifier",
        T_INT => "an integer literal",
        T_FLOAT => "a float literal",
        T_STR => "a string literal",
        T_BSTR => "a byte-string literal",
        T_BYTE => "a byte literal",
        T_UNDERSCORE => "`_`",
        T_KW_FN => "`fn`",
        T_KW_LET => "`let`",
        T_KW_MUT => "`mut`",
        ...
        T_KW_RETURN => "`return`",
        T_KW_PUB => "`pub`",
        T_KW_RESERVED => "a reserved keyword",
        T_LPAREN => "`(`",
        T_RPAREN => "`)`",
        T_LBRACE => "`{`",
        T_RBRACE => "`}`",
